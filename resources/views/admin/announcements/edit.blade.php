            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="5" class="mt-1 w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-950">{{ old('body', $announcement->body) }}</textarea>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <select id="audience" name="audience" class="mt-1 w-full rounded-xl border-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-950">
                                @foreach (['all', 'admin', 'lecturer', 'student'] as $audience)
                                    <option value="{{ $audience }}" @selected(old('audience', $announcement->audience) === $audience)>{{ ucfirst($audience) }}</option>
                                @endforeach
                            </select>
                        ...
                        <label class="mt-7 inline-flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                            <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 dark:border-slate-700" @checked(old('is_active', $announcement->is_active)) />
                            {{ __('Active') }}
                        </label>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="starts_at" name="starts_at" type="datetime-local" class="mt-1 block w-full" :value="old('starts_at', optional($announcement->starts_at)->format('Y-m-d\TH:i'))" />
                            <x-input-error :messages="$errors->get('starts_at')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="ends_at" :value="__('Ends At')" />
                            <x-text-input id="ends_at" name="ends_at" type="datetime-local" class="mt-1 block w-full" :value="old('ends_at', optional($announcement->ends_at)->format('Y-m-d\TH:i'))" />
